...
describe("Heading", () => {
  ...
  describe("children 이 없을 때", () => {
    beforeEach(() => {
      renderResult = render(<Heading data={"제목"} level={1} />);
    });
    commonTest();
  ...
  function commonTest() {
    test("ref 는 HTMLHeading 을 가리킨다.", () => {
      ...
      const ref = jest.fn();
      rerender(<Heading data={"제목"} level={1} ref={ref} />);
